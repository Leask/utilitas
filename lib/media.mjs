import { BUFFER, convert, FILE, getTempPath } from './storage.mjs';
...
const convertAudio = async (input, options) => {
    ...
    const { content, cleanup } = await convert(input, {
        ...options || {}, expected: FILE, withCleanupFunc: true,
    });
    const conv = await getFfmpeg(content);
    const output = options?.output || getTempPath({
        sub: options?.suffix
            ? `${hash(JSON.stringify({ content, options }))}.${options.suffix}`
            : null,
    });
    ...
    options?.input === FILE || await cleanup();
    return await convert(output, {
        expected: BUFFER, cleanup: true, ...options || {}, input: FILE,
    });
};
