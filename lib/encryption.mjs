...
const aesCreateKey = (options) => {
    ...
    return { buffer: key, base64: options?.key || base64Encode(key, true) };
};
...
const aesCreateIv = (options) => {
    ...
    return { buffer: iv, base64: options?.iv || base64Encode(iv, true) };
};

const _aesDecodeAuthTag = (options) => ({
    buffer: base64Decode(options?.authTag, true), base64: options?.authTag,
});
