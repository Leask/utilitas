import {
    assertEmail, ensureArray, log as _log, need, throwError, trim, uniqueArray,
    which
} from './utilitas.mjs';
...
const init = async (options) => {
    if (options) {
        ...
        switch (provider) {
            case 'MAILGUN':
                const formData = await need('form-data');
                const mailgun = await need('mailgun.js');
                domain = options.domain;
                engine = new mailgun(formData);
                client = engine.client(options);
                break;
            case 'MAILJET':
                engine = await need('node-mailjet');
