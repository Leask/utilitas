...
import {
    ensureArray, ensureInt, ensureString, insensitiveCompare, log as uLog
} from './utilitas.mjs';
...
const uLg = (cnt, opt) => uLog(cnt, import.meta.url, { time: 1, ...opt || {} });
...
const init = async (options) => {
    if (options) {
        ...
        if (!options?.silent) {
            uLg(`Sending logs via bot, chatId: ${chatIds.join(', ')}.`);
        }
        log = rawLog;
        await loop(
