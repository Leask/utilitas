...
const shift = async (k, o) => await call(SHIFT, { args: [key, o] }, o);
const pop = async (k, o) => await call(POP, { args: [key, o] }, o);
const flush = async (k, o) => await shift(k, { length: Infinity, ...o || {} });
const delKey = async (k, o) => await call(DELKEY, { args: k, options: o }, o);
const functions = {};
...
const takeOut = (key, options) => {
    assertKey(key);
    const resp = [];
    while (storage[key]?.value?.length && resp.length < options?.length || 1) {
        resp.push(storage[key].value[options.method]());
    }
    return isSet(options?.length, true) ? resp : resp[0];
};
