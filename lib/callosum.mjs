...
const _set = (k, v, o) => {
    const curValue = storage?.[k]?.value;
    let value = ((o?.push || o?.unshift) && ensureArray(curValue)) ||
        (o?.assign && (Object.isObject(curValue) ? curValue : { _: v })) || v;
    if (o?.push) { value.push(v); } else if (o?.unshift) { value.unshift(v); }
...
const queue = (key, value, options) => set(key, value, {
    push: true, shift: options?.length, ...options || {}
});
