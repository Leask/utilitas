...
import {
    __, basename, insensitiveCompare, log as _log, need
} from './utilitas.mjs';
...
const [GET, HTTP_PORT, HTTP, HTTPS, SSL_RESET, HTTP01, termsOfServiceAgreed]
    = ['GET', 80, 'http', 'https', 'SSL_RESET', 'http-01', true];
...
const challengeCreateFn = fn => async (authz, challenge, keyAuthorization) => {
    assertChlnge(challenge.type);
    return await fn(getPathByToken(challenge.token), keyAuthorization);
};

const challengeRemoveFn = fn => async (authz, challenge, keyAuthorization) => {
    assertChlnge(challenge.type);
    return await fn(getPathByToken(challenge.token));
};
...
const ensureCert = async (domain, challengeCreate, challengeRemove, options) => {
    ...
    if (!cert) {
        ...
        cert = await client.auto({
            ...
            challengeCreate: challengeCreateFn(challengeCreate),
            challengeRemove: challengeRemoveFn(challengeRemove),
        });
...
const init = async (domain, options) => {
    ...
    httpd.listen(HTTP_PORT, options?.address || '', () => {
        const { add } = getAddress(HTTP, domain, httpd);
        log(`SSL-Challenge HTTP Server started at ${add}.`);
    }); // https://letsencrypt.org/docs/rate-limits/
    const timer = await loop(() => ensureCert(domain,
...
// @todo: debug!? still need this? by @Leask
on(SSL_RESET, resetCurCert);

export default init;
export { _NEED, createCsr, ensureCert, getCert, init };
