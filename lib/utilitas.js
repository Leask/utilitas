...
const ensureInt = (any, options) => {
    ...
    int = isNaN(int) ? 0 : int;
    int = options.min && int < options.min ? options.min : int;
    int = options.max && int > options.max ? options.max : int;
    return int;
};
...
const parseVersion = (verstr) => {
    ...
    if (result.version) {
        const verArr = result.version.split('.');
        while (verArr.length < 5) { verArr.push(0); }
        let sum = 0;
        let stVal = 0;
        while (verArr.length) {
            result.normalized += Math.pow(10, stVal++ * 3) * verArr.pop();
        }
    }
    return result;
};
