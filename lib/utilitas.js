    ...
    switch (getType(any)) {
        case 'Object':
            ...
            for (let i in any) {
                const subPath = path || [];
                subPath.push(i);
                const newKey = func(i, any[i], subPath);
                assert(!strict || newKey, `Error maping key: '${i}'.`, 400);
                resp[newKey || i] = mapKeys(any[i], func, strict, subPath);
            }
            break;
        case 'Array':
            resp = [];
            any.map((x, i) => {
                const subPath = path || [];
                subPath.push(i);
                resp.push(mapKeys(x, func, strict, subPath));
            });
    }
    return resp;
};
...
const ensureInt = (any, options) => {
    ...
    return options.pad > 0 ? pad(int, options.pad, { pad: '0' }) : int;
};

const ensureArray = (array) => {
    return isSet(array) ? (Array.isArray(array) ? array : [array]) : []
};
...
const extract = function() {
    ...
    for (let i = 0; i < arguments.length; i++) {
        if (i) { r = r && r[arguments[i]] || null; } else { r = arguments[i]; }
    }
    return r;
};
...
const getDateByUnixTimestamp = (timestamp) => {
    return new Date(~~timestamp * 1000);
};

const getUnixTimestampByDate = (date) => {
...
const range = (from, to, options) => {
    const errRange = 'Invalid Range.';
    options = options || {};
    options.base = ensureInt(options.base, { min: 0 });
    options.step = ensureInt(options.step, { min: 1 });
    assert((from = ensureInt(from)) && (to = ensureInt(to)), errRange, 400);
