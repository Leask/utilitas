...
const assembleUrl = (url, componens) => {
    ...
    for (let i in componens || []) {
        (Array.isArray(componens[i]) ? componens[i] : [componens[i]]).map(x => {
...
const modLog = (content, filename) => {
    ...
    const args = [`[${path.basename(
        filename || __filename
    ).replace(/\.js$/, '').toUpperCase()}]${isErr ? '' : (' ' + content)}`];
...
const mergeAtoB = (a = {}, b = {}) => {
    for (let i in a) { if (!utilitas.isUndefined(a[i])) { b[i] = a[i]; } }
};
