...
// Fast image metadata parser.
// Currently supported formats: PNG, JPEG, WEBP.
const getImageSize = (buf) => {
    ...
    if (buf[0] === 0x52 && buf[1] === 0x49 && buf[2] === 0x46 && buf[3] === 0x46 &&
        ...
        const type = buf.toString('utf8', 12, 16);
        if (type === 'VP8 ') return [buf.readUInt16LE(26) & 0x3fff, buf.readUInt16LE(28) & 0x3fff];
        if (type === 'VP8L') return [1 + (((buf[22] & 0x3f) << 8) | buf[21]), 1 + (((buf[24] & 0xf) << 10) | (buf[23] << 2) | ((buf[22] & 0xc0) >> 6))];
        if (type === 'VP8X') return [1 + buf.readUIntLE(24, 3), 1 + buf.readUIntLE(27, 3)];
    }
    throwError('Unsupported image format.', 400);
};
