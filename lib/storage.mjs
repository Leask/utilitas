...
const relative = (url, sub) => path.join(utilitas.__(url).__dirname, sub);
...
const readFile = async (filename, options) => await fs.promises.readFile(
...
const writeFile = async (name, data, opts) => await fs.promises.writeFile(
...
const writeIni = async (filename, data, options) => await writeFile(
    filename, ini.stringify(data || {}, options), options
);
