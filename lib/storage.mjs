import {
    base64Decode, base64Encode, ensureString, ignoreErrFunc, mergeAtoB,
    throwError, trim, which,
} from './utilitas.mjs';

import { constants as consts, promises as fs, readSync, unlink } from 'fs';
...
const convert = async (any, options) => {
    ...
    const [ori, input, expected] = [any, options?.input, options?.expected].map(
        x => ensureString(x || BUFFER, { case: 'UP' })
    );
    if (input !== expected) {
    ...
    input === FILE && expected !== FILE && options?.cleanup && await tryRm(ori, options);
    return any;
};
