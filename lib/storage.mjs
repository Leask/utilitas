import {
    assertBuffer, base64Decode, base64Encode, ensureString, extract,
    ignoreErrFunc, mergeAtoB, need, throwError, trim, voidFunc, which,
} from './utilitas.mjs';

import { basename, extname, join } from 'path';
import { constants as consts, promises as fs, readSync } from 'fs';
import { defaultAlgorithm, hash } from './encryption.mjs';
import { deflate as __zip, unzip as __unzip } from 'node:zlib';
...
const mergeFile = async (data, options) => {
    ...
    const file = Buffer.concat(content);
    data?.hash && assert(
