...
const subconscious = [{
    ...
    run: true, priority: -8870, name: 'config', func: async (ctx, next) => {
        switch (ctx.cmd.cmd) {
            case 'set':
                try {
                    const _config = {
...
const parseArgs = args => {
    ...
    const result = {};
    tokens.map(x => result[x.name] = bot._.args[x.name]?.volidate
        ? bot._.args[x.name].volidate(values[x.name])
        : values[x.name]);
    return result;
};
