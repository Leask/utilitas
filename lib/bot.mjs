    ...
    run: true, name: 'laws', func: async (bot) => {
        bot.use(async (ctx, next) => {
            [...questions, {
                ...
                x => iCmp(x, ctx.text) && (ctx.end = ctx.replyWithMarkdown(s.a))
            ));
            await next();
        });
    },
    ...
    run: true, name: 'authenticate', func: async (bot) => {
        bot.private && bot.use(async (ctx, next) => {
            ...
            if (bot.magicWord && iHas(bot.magicWord, ctx.text)) {
                ...
                ctx.replyWithMarkdown('😸 You are now allowed to talk to me.');
                ctx.text = HELLO;
                return await next();
            }
            ...
            if (bot.homeGroup && await ignoreErrFunc(
                async () => await bot.telegram.getChatMember(bot.homeGroup, ctx.from.id)
            )) {
                return await next();
            }
            ...
            ctx.end = ctx.replyWithMarkdown('😿 Sorry, I am not allowed to talk to strangers.');
            await next();
        });
    },
}, {
