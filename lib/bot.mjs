...
import { BUFFER, isTextFile, tryRm } from './storage.mjs';
    ...
    run: true, priority: -8910, name: 'speech-to-text', func: async (ctx, next) => {
        if (ctx._.speech?.stt && ctx.m.voice?.mime_type === 'audio/ogg') {
            ...
            try {
                ...
                if (bot._.supportedMimeTypes.has('audio/wav')) {
                    ctx.collect({
                        mime_type: 'audio/wav', url,
                        data: await convertAudioTo16kNanoPcmWave(file, { input: BUFFER, expected: 'BASE64' }),
                    }, 'PROMPT');
                } else {
