...
const subconscious = [{
    ...
    run: true, priority: -8950, name: 'subconscious', func: async (ctx, next) => {
        ...
        if ((ctx.text || ctx.msg.voice || ctx.msg.poll
            || ctx.update[ctx.type].data) && ctx.messageId) { await next(); }
    ...
    run: true, priority: -8920, name: 'speech-to-text', func: async (ctx, next) => {
        ...
        try {
            ...
            ctx.collect(ignoreErrFunc(async () => await ctx._.speech?.stt(
                await getFile(
    ...
    run: true, priority: -8910, name: 'callback', func: async (ctx, next) => {
        if (ctx.type === 'callback_query') {
            ...
            if (cb?.text) { ctx.collect(cb.text, 'TEXT_BUTTON') } else {
                return await ctx.er(
            ...
            ctx.collect(lines([
                'Question:', ctx.update.message.poll.question, 'Options:',
    ...
    run: true, priority: -8900, name: 'web', func: async (ctx, next) => {
        ctx.msg?.entities?.some(
            entitie => entitie.type === 'url'
        ) && await ctx.ok(EMOJI_LOOK);
        for (let i in ctx.msg?.entities) {
            if (ctx.msg.entities[i]?.type !== 'url') { continue; }
            const content = await ignoreErrFunc(async () => (
                await distill(ctx.text.substr(e.offset, e.length))
            )?.summary);
    ...
    run: true, priority: -8890, name: 'commands', func: async (ctx, next) => {
        ...
        const cmd = trim(
            arrText[0]?.replace(COMMAND_REGEXP, '$1'), { case: 'LOW' }
        );
        if (cmd) {
            ctx.cmd = {
                cmd, args: lines([
                    arrText[0].replace(COMMAND_REGEXP, '$3').trim(),
                    ...arrText.slice(1)
                ])
            };
