...
const [BUFFER, sampleRateHertz] = ['BUFFER', { sampleRateHertz: 48000 }];
...
const subconscious = [{
    run: true, priority: -8960, name: 'broca', func: async (ctx, next) => {
        ...
        ctx.er = async (m, options) => {
            if (m) {
                await ctx.ok(m?.message ? `[ERROR] ${m?.message}` : m, options);
                log(m);
            }
            return ctx.end = true;
        };
    ...
    run: true, priority: -8950, name: 'subconscious', func: async (ctx, next) => {
        ...
        ((
            !ctx.text && !msg.voice && !ctx.update[ctx.type].data
        ) || !ctx.messageId) && await ctx.er()
        await next();
    },
}, {
    ...
    run: true, priority: -8920, name: 'stt', func: async (ctx, next) => {
        ...
        } catch (err) { await ctx.er(err); }
        await next();
    },
}, {
    run: true, priority: -8920, name: 'buttons', func: async (ctx, next) => {
    ...
    run: true, priority: -8910, name: 'overwrite', func: async (ctx, next) => {
        if (ctx.end) { return await next(); }
