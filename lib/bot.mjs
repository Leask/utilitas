...
const subconscious = [{
        ...
        if (ctx.chatType !== PRIVATE && (ctx.msg.entities || []).some((e, k) => {
            switch (e.type) {
                ...
                case bot_command:
                    ...
                    if (!~~ctx.msg.entities[k].offset) {
                        ctx.text = ctx.text.split('');
                        ctx.text.splice(~~e.offset + _cmd[0].length, ~~e.length - _cmd[0].length);
                        ctx.text = ctx.text.join('');
                        ctx.msg.entities[k].offset = -1;
                    }
                    return isMe;
            }
        })) {
            if (!~~ctx.msg.entities[0].offset) {
                ctx.text = ctx.text.substr(~~ctx.msg.entities[0].length + 1);
            }
            ctx.chatType = MENTION;
        }
        if ((ctx.text || ctx.msg.voice || ctx.msg.poll || ctx.update[ctx.type].data)
    ...
    run: true, priority: -8900, name: 'web', func: async (ctx, next) => {
        let urls = ctx.msg?.entities?.map(
            e => e.type === 'url' ? ctx.text.substr(e.offset, e.length) : null
        ).filter(x => x) || [];
        Object.keys(urls).length && await ctx.ok('👀');
        ctx.urls = {};
        for (let url of urls) {
            ctx.urls[url] = await ignoreErrFunc(
                async () => (await distill(url))?.summary
            );
        }
        await next();
    },
