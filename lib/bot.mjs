...
import {
    ...
    ignoreErrFunc, insensitiveCompare, insensitiveHas, lastItem, log as _log,
    need, parseJson, prettyJson, splitArgs, timeout, trim, which,
} from './utilitas.mjs';
...
const initSql = {
    [MYSQL]: [[
        cleanSql(`CREATE TABLE IF NOT EXISTS ?? (
            ...
            INDEX (\`message_id\`),
            INDEX (\`received\`(768)),
            INDEX (\`received_text\`(768)),
            INDEX (\`response\`(768)),
            INDEX (\`response_text\`(768)),
            INDEX (\`collected\`(768)),
            FULLTEXT INDEX (\`distilled\`),
            INDEX (\`created_at\`),
            INDEX (\`updated_at\`)
        ...
        switch (bot._?.database?.provider) {
            case MYSQL:
                result = await bot._.database?.client?.query?.(
                    ...
                    + 'FROM ?? WHERE `chat_id` = ? HAVING relevance > 0 '
                    + 'ORDER BY `relevance` DESC, `id` DESC '
                    + `LIMIT ${SEARCH_LIMIT} OFFSET ?`,
                    [keyWords, table, ctx.chatId, offset]
                );
                break;
            case POSTGRESQL:
                globalThis.debug = 2;
                ...
                result = await bot._.database?.client?.query?.(
                    ...
                    + ` ORDER BY distilled_vector <-> $2 LIMIT ${SEARCH_LIMIT}`
                    + ` OFFSET $3`,
                    [ctx.chatId, await encodeVector(vector), offset]
                );
                break;
        }
        for (let i in result) {
            const received = JSON.parse(result[i].received);
            const lastPage = ~~i === result.length - 1;
            received.message.date = new Date(received.message.date * 1000);
            const content = lines([
                ...result[i].response_text ? [
                    `- ↩️ ${compactLimit(result[i].response_text)}`
                ] : [],
                `- ${getTimeIcon(received.message.date)} `
                + `${received.message.date.toLocaleString()}`,
            ]);
            ctx.done.push(await reply(ctx, true, content, {
                reply_to_message_id: result[i].message_id,
            ...
            lastPage && await editMessageText(
                ...
                getExtra(ctx, {
                    reply_to_message_id: null, buttons: [{
                        label: '🔍 More',
