...
const [BUFFER_ENCODE, BINARY_STRINGS] = [{ encode: BUFFER }, [ON, OFF]];
...
const subconscious = [{
    run: true, priority: -8960, name: 'broca', func: async (ctx, next) => {
        ...
        ctx.ok = async (message, options) => {
            ...
            for (let i in pages) {
                ...
                lastPage || await ctx.timeout(ctx);
            }
            return ctx.done;
        };
        ctx.er = async (m, opts) => {
    ...
    run: true, priority: -8930, name: 'authenticate', func: async (ctx, next) => {
        if (                                                                    // if chatType is not in whitelist, exit.
            !insensitiveHas(ctx._.chatType, ctx.chatType)
