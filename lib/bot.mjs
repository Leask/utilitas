...
const subconscious = [{
    run: true, priority: -8960, name: 'subconscious', func: async (bot) => {
        bot.use(async (ctx, next) => {
            ...
            switch (ctx.type) {
                case 'my_chat_member':
                    if (ctx.update[ctx.type].new_chat_member.user === ctx.botInfo.id
                        && ctx.update[ctx.type].new_chat_member.status === 'left') {
                        ctx.end = true;
                    } else { ctx.text = HELLO; }
                case 'message': case 'edited_message': case 'channel_post':
                    ...
                    ctx.text = ctx.text || ctx.update[ctx.type].text;
                    ctx.messageId = ctx.update[ctx.type].message_id;
                    ctx.end = ctx.end || (!ctx.text && !ctx.update[ctx.type].voice) || !ctx.messageId;
