...
const subconscious = [{
    run: true, priority: -8960, name: 'subconscious', func: async (bot) => {
        bot.use(async (ctx, next) => {
            ...
            if (ctx.update.message) {
                ...
                msg = ctx.update[ctx.type].message;
                const data = parseJson(ctx.update[ctx.type].data);
                if (data?.text) { ctx.text = data.text; } else {
                    ctx.end = true;
                    log(`Unsupported button data.`);
                }
            } else if (ctx.update.channel_post) {
                ctx.type = 'channel_post';
            } else if (ctx.update.edited_message) {
                ctx.type = 'edited_message';
            } else if (ctx.update.my_chat_member) {
                ctx.type = 'my_chat_member';
    ...
    run: true, priority: -8950, name: 'enrich', func: async (bot) => {
        bot.use(async (ctx, next) => {
    ...
    run: true, priority: -8940, name: 'laws', func: async (bot) => {
        bot.use(async (ctx, next) => {
    ...
    run: true, priority: -8930, name: 'authenticate', func: async (bot) => {
    ...
    run: true, priority: -8920, name: 'stt', func: async (bot) => {
        bot.speech?.stt && bot.use(async (ctx, next) => {
            if (ctx.end || ctx.message?.voice?.mime_type !== 'audio/ogg') {
                return await next();
            }
            try {
