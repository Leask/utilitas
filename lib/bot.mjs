...
import { MYSQL, POSTGRESQL, cleanSql } from './dbio.mjs';
...
const initSql = {
    [MYSQL]: [[
        cleanSql(`CREATE TABLE IF NOT EXISTS ?? (
            ...
            \`receivedText\`    TEXT NOT NULL,
            \`response\`        TEXT NOT NULL,
            \`responseText\`    TEXT NOT NULL,
            \`collected\`       TEXT NOT NULL,
            \`distilled\`       TEXT NOT NULL,
            \`distilledVector\` TEXT NOT NULL,
            ...
            INDEX (\`messageId\`),
            INDEX (\`received\`(768)),
            INDEX (\`receivedText\`(768)),
            INDEX (\`response\`(768)),
            INDEX (\`responseText\`(768)),
            INDEX (\`collected\`(768)),
            INDEX (\`distilled\`(768)),
            INDEX (\`createdAt\`),
            INDEX (\`updatedAt\`)
    ...
    [POSTGRESQL]: [[
        cleanSql(`CREATE TABLE IF NOT EXISTS ${table} (
            ...
            collected       TEXT NOT NULL,
            distilled       TEXT NOT NULL,
            distilledVector VECTOR(1536) NOT NULL,
            createdAt       TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
            updatedAt       TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
        )`)
    ], [
        `CREATE INDEX IF NOT EXISTS chatId_index ON ${table} (chatId)`,
    ], [
        `CREATE INDEX IF NOT EXISTS chatType_index ON ${table} (chatType)`,
    ], [
        `CREATE INDEX IF NOT EXISTS messageId_index ON ${table} (messageId)`,
    ], [
        `CREATE INDEX IF NOT EXISTS received_index ON ${table} (received)`,
    ], [
        `CREATE INDEX IF NOT EXISTS receivedText_index ON ${table} (receivedText)`,
    ], [
        `CREATE INDEX IF NOT EXISTS response_index ON ${table} (response)`,
    ], [
        `CREATE INDEX IF NOT EXISTS responseText_index ON ${table} (responseText)`,
    ], [
        ...
        `CREATE INDEX IF NOT EXISTS distilledVector_index ON ${table} USING hnsw(distilledVector vector_cosine_ops)`,
    ], [
...
const memorize = async (ctx) => {
    ...
    const response = ctx.done[ctx.done.length - 1] || {};
    const responseText = response?.text || '';
    const collected = ctx.collected.filter(x => String.isString(x.content));
    const event = {
        id, chatId: ctx.chatId, chatType: ctx.chatType, messageId: ctx.messageId,
        received: JSON.stringify(received), receivedText,
        response: JSON.stringify(response), responseText,
        collected: JSON.stringify(collected), distilled: trim([
            receivedText, responseText, ...collected.map(x => x.content)
        ].join(' ').replace(/\s+/g, ' ')),
    };
    await ignoreErrFunc(async () => {
        event.distilledVector = bot._.embedding
            ? await bot._.embedding(event.distilled) : [];
        switch (bot._.database.provider) {
            case MYSQL:
                event.distilledVector = JSON.stringify(event.distilledVector);
                break;
        }
        await bot._.database?.client?.upsert?.(table, event);
