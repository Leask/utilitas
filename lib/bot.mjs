...
const subconscious = [{
    ...
    run: true, name: 'laws', func: async (bot) => {
        bot.use(async (ctx, next) => {
            [...questions, {
                ...
                async x => iCmp(x, ctx.text) && (ctx.end = await ctx.ok(s.a))
            ));
            await next();
        });
    },
    ...
    run: true, name: 'ai', func: async (bot) => {
        bot.ai && bot.use(async (ctx, next) => {
            ...
            const [msgs, onProgress, pms, matchReg, selected]
                = [{}, { onProgress: true }, [], /^\/([^\ ]*)(.*)$/ig, new Set()];
            let [lastMsg, lastSent, firstBot] = ['', 0, null];
            const packMsg = (options) => {
                const packed = [];
                for (let name of selected.size ? selected : [firstBot]) {
                    packed.push([
                        ...selected.size ? [`🤖️ ${name}:`] : [],
                        options?.onProgress ? (
            ...
            for (let name in bot.ai) {
                ...
                (iCmp(cmd, name) || cmd === '*') && selected.add(name);
            }
            selected.size
