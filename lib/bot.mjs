...
const [
    ...
    COMMAND_REGEXP, MESSAGE_LENGTH_LIMIT, MESSAGE_SOFT_LIMIT
] = [
...
const paging = message => {
    ...
    submit()
    return pages.map((p, i) => lines(p).trim() + (
...
const subconscious = [{
    run: true, priority: -8960, name: 'broca', func: async (ctx, next) => {
        ...
        ctx.ok = async (message, options) => {
            ...
            if (options?.onProgress && !ctx.done.length) {                      // progress, 1st, reply text
                ...
                ctx.done.push(await ctx.telegram.editMessageText(
                    ...
                    { parse_mode: md ? 'Markdown' : 'Text', ...extra }
                ));
        ...
        ctx.paging = async (message, options) => {
            ...
            for (let i in pages) {
                ctx.done.push(await ctx.ok(pages[i], options));
                (i < pages.length - 1) && (await timeout(ctx.limit));
            }
            return ctx.done;
        };
        ctx.er = async (m, opts) => {
