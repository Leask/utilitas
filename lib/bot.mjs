...
const sessionGet = async chatId => {
    ...
    sessions[chatId].hello || (sessions[chatId].hello = bot._.hello || HELLO);
...
const subconscious = [{
    run: true, priority: -8960, name: 'broca', func: async (ctx, next) => {
        ctx.done = [];
        ctx.hello = str => ctx.text = str || ctx.session.hello;
    ...
    run: true, priority: -8870, name: 'configuration', func: async (ctx, next) => {
        ...
        'Example 1: /set --`OPTION_1` `VALUE_1` --`OPTION_2`',
        'Example 2: /set -`SHORT_1` `VALUE_1` -`SHORT_2`',
    ]),
}, {
