...
const [ // https://limits.tginfo.me/en
    ...
    EMOJI_LOOK, EMOJI_BOT, logOptions,
] = [
...
const editMessageText = async (ctx, md, lastMsgId, text, extra) => {
    try {
        ...
        assert(isMarkdownError(e), e, 500);
        await ctx.timeout(ctx);
    }
    return await ignoreErrFunc(
...
const subconscious = [{
    run: true, priority: -8960, name: 'broca', func: async (ctx, next) => {
        ...
        ctx.ok = async (message, options) => {
            ...
            const paged = options?.paging || pages.length > 1;
            const md = options?.md && !paged;
            ...
            for (let i in pages) {
                ...
                if (options?.onProgress && !pageIds[~~i]) {                     // progress: 1st, reply text
                    ctx.done.push(await reply(ctx, false, pages[i], extra));
                ...
                (~~i < pages.length - 1) && (await ctx.timeout(ctx));
            }
            return ctx.done;
        };
        ctx.er = async (m, opts) => {
    ...
    run: true, priority: -8950, name: 'subconscious', func: async (ctx, next) => {
        ...
        ctx.chatType !== PRIVATE && [{
            text: ctx.msg.text, entities: ctx.msg.entities,
        }, {
            text: ctx.msg.caption, entities: ctx.msg.caption_entities,
        }].some(g => (g.entities || []).some(e => {
            let target;
            switch (e.type) {
                case MENTION:
                    target = g.text.substr(~~e.offset + 1, ~~e.length - 1);
                    break;
                case bot_command:
                    target = g.text.substr(~~e.offset, ~~e.length).split('@')[1];
                    break;
            }
    ...
    run: false, priority: 0, name: 'debug', func: async (ctx, next) => {
        const lorem = new (await need('lorem-ipsum')).LoremIpsum();
        // testing basic reply
        // ctx.ok(lorem.generateParagraphs(3));
        // testing markdown reply
        // ctx.ok('[leaskh.com](https://leaskh.com)', { md: true });
        // testing incomplete markdown reply
        // ctx.ok('_8964', { md: true });
        // test paging reply with progress
        let text = '';
        for (let i = 0; i < 3; i++) {
            await ctx.ok(text += lorem.generateParagraphs(5), { onProgress: 1 });
            await ctx.timeout();
        }
        await ctx.ok(text += lorem.generateParagraphs(5), { md: true });
        // test pagebreak
        // await ctx.timeout();
        // await ctx.ok(lorem.generateParagraphs(10), { md: true, pageBreak: true });
    },
}, {
