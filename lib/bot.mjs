...
import {
    ...
    log as _log, need, parseJson, prettyJson, which,
} from './utilitas.mjs';
...
const subconscious = [{
    ...
    run: true, priority: -8890, name: 'cmd', func: async (ctx, next) => {
        const matchReg = /^\/([^\ ]*)(.*)$/ig;
        if (!ctx.text) { return await next(); }
        const arrText = ctx.text.split('\n');
        const cmd = arrText[0]?.replace(matchReg, '$1');
        for (let c of bot._.cmds) {
            if (insensitiveCompare(cmd, c.command)) {
                ctx.cmd = { cmd: c.command };
                break;
            }
        }
        if (ctx.cmd) {
            arrText[0] = arrText[0].replace(matchReg, '$2').trim();
            ctx.cmd.args = arrText.join('\n');
        }
        await next();
    },
}, {
    run: true, priority: -8880, name: 'help', func: async (ctx, next) => {
        ...
        for (let i in ctx._.skills) {
            ...
            if (countKeys(ctx._.skills[i].cmds)) {
                _help.push(Object.keys(ctx._.skills[i].cmds).map(
                    x => `- /${x}: ${ctx._.skills[i].cmds[x]}`
                ).join('\n'));
            }
            if (countKeys(ctx._.skills[i].args)) {
                _help.push(Object.keys(ctx._.skills[i].args).map(x => {
            ...
            _help.length && help.push([`*${i.toUpperCase()}*`, ..._help].join('\n'));
        }
        await ctx.ok(help.join('\n\n'));
    }, cmds: {
    ...
    run: true, priority: -8880, name: 'test', func: async (ctx, next) => {
        await ctx.ok('😄');
        await next();
    }, cmds: {
        'xxxx': 'yyyyy'
    }, help: 'This is a test skill.', args: {
        ttsonx: { type: 'string', short: 'a', default: 'ssss', desc: 'Enable XXXX.' },
        ttsoffx: { type: 'string', short: 'a', desc: 'Disable YYYYY.' },
    },
}, {
...
const parseArgs = args => _parseArgs({ args, options: bot._.args, tokens: true });
...
const init = async (options) => {
    if (options) {
        ...
        if (isPrimary) {
            ...
            parseArgs([]); // Volidate args options.
            bot.launch();
