import { convert, getTempPath, tryRm } from './storage.mjs';
...
const [STREAM, BASE64, FILE, clients, languageCode, audioEncoding, suffix, SPEAKER, cleanup]
    = ['STREAM', 'BASE64', 'FILE', {}, 'en-US', 'OGG_OPUS', 'ogg', 'SPEAKER', true];
...
const init = async (options) => {
    if (options) {
        ...
        const engine = ensureString(options?.engine, { case: 'UP' });
        switch (engine) {
            case 'OPENAI':
                clients._engine = engine;
...
const ttsOpenAI = async (input, options) => {
    ...
    const content = await clients.tts.create({
        model: 'tts-1-hd', voice: 'nova', response_format: 'opus', input,
        ...options,
    });
...
const sttOpenAI = async (audio, options) => {
    ...
    const file = await convert(audio, {
        input: options?.input, expected: STREAM, errorMessage,
    });
    const result = await clients.stt.create({
        file, model: 'whisper-1', response_format: 'text', ...options,
    });
    ensureString(options?.input, { case: 'UP' }) !== FILE && await tryRm(file.path);
    return result;
};
...
const tts = async (text, options) => {
    ...
    else if (clients?.tts && clients._engine === 'OPENAI') { engine = ttsOpenAI; }
    else if (clients?.tts && clients._engine === 'GOOGLE') { engine = ttsGoogle; }
...
const stt = async (audio, options) => {
    let engine;
    if (clients?.stt && clients._engine === 'OPENAI') { engine = sttOpenAI; }
    else if (clients?.stt && clients._engine === 'GOOGLE') { engine = sttGoogle; }
