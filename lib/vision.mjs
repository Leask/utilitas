...
const parseOfficeFile = async (input, options) => {
    officeParser || (officeParser = (
        await need('office-text-extractor')
    ).getTextExtractor());
    // PdfExtractor, DocExtractor, PptExtractor, ExcelExtractor
    const extractor = options?.extractor || 'extractText';
    return await officeParser[extractor]({
        input, type: ensureString(options?.input || BUFFER, { case: 'LOW' })
    });
};
