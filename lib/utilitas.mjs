...
const assertEmail = (email, ms, status, o) => {
    return assert(verifyEmail(email), ms || 'Invalid email.', status || 400, o);
};

const assertUuid = (uuid, msg, status, opt) => {
    return assert(verifyUuid(uuid), msg || 'Invalid UUID.', status || 400, opt);
};

const assertUrl = (url, msg, status, opt) => {
    return assert(verifyUrl(url), msg || 'Invalid URL.', status || 400, opt);
};

const assertSet = (value, msg, status, opt) => {
    return assert(isSet(value), msg || 'Undefined value.', status || 400, opt);
};

const assertDate = (time, msg, status, opt) => {
    return assert(Date.isDate(time, true), msg || invalidTime, status || 400, opt);
};

const assertFunction = (fn, msg, status, o) => {
    return assert(Function.isFunction(fn), msg || 'Invalid Function.', status || 400, o);
};
...
const which = async (pack) => {
    ...
    const data = Object.isObject(pack) ? pack : await storage.readJson(pack);
...
const renderCode = (code, options) => {
    ...
    return arrCode.map(x => `${String(++i).padStart(bits, '0')}${s}${ensureString(
        x, { singleLine: true }
    )}`).join('\n');
};
