...
const inBrowser = () => { try { return navigator; } catch (e) { } };
...
const getRandomIndexInArray = (array, options) => {
    ...
    const result = randomArray(range(0, array.length - 1)).slice(0, count || 1);
...
const prettyJson = (object, opt) => {
    ...
    opt?.code && (resp = renderCode(resp, opt));
...
const renderText = (text, options) => {
    ...
    arrText.map(line => {
        ...
        if (codeBlock && !code.length) { // start code block
            !options?.noCode && options?.extraCodeBlock && result.push(line.replace(/`/ig, '\\`'));
            code.push(line);
        } else if (codeBlock && code.length) { // end code block
            if (!options?.noCode) {
                result.push(code.shift(), renderCode(code, options), line);
                options?.extraCodeBlock && result.push(line.replace(/`/ig, '\\`'));
            }
            code.length = 0;
