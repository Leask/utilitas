...
const fileURLToPath = _fileURLToPath || (url => new URL('', url).pathname);
...
const assertDate = (time, msg, status, opt) => {
    return assert(isDate(time, true), msg || invalidTime, status || 400, opt);
};

const assertFunction = (fn, msg, status, o) => {
    return assert(isFunction(fn), msg || 'Invalid Function.', status || 400, o);
};
