...
const init = async (options) => {
    ...
    switch (provider) {
        ...
        case GEMINI:
            ...
            clients[provider] = { image: client.models.generateImages };
            break;
        default:
...
const image = async (prompt, options) => {
    ...
    switch (provider) {
        ...
        case GEMINI:
            ...
            assert(!resp?.error && resp.generatedImages?.filter(
                x => !x.raiFilteredReason
            ).length, resp?.error?.message || resp.generatedImages?.find(
                x => x.raiFilteredReason
