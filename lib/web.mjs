...
const CACHE_VARIANT_HEADERS = [
    'accept', 'accept-language', 'accept-encoding', 'range', 'content-type',
];
const CACHE_PRIVATE_VARIANT_HEADERS = ['authorization', 'cookie'];
const getJson = async (u, o) => await get(u, { encode: _JSON, ...o || {} });
const getParsedHtml = async (u, o) => await get(u, { encode: _PARSED, ...o || {} });
const checkSearch = () => jinaApiKey;
...
const normalizeHeaders = (...items) => {
    const headers = new Headers();
    for (const item of items) {
        if (!item) { continue; }
        for (const [key, value] of new Headers(item)) {
...
const getCacheVariantHeaders = headers => {
    const cacheHeaders = {};
    for (const key of CACHE_VARIANT_HEADERS) {
        const value = headers.get(key);
        if (value) { cacheHeaders[key] = value; }
    }
    for (const key of CACHE_PRIVATE_VARIANT_HEADERS) {
        const value = headers.get(key);
        if (value) { cacheHeaders[key] = sha256(value); }
    }
    return cacheHeaders;
};
...
const get = async (url, options) => {
    ...
    const requestHeaders = normalizeHeaders(
        defFetchOpt.headers, options.fetch?.headers
    );
    const cacheKey = JSON.stringify({
        method: ensureString(options.fetch?.method || 'GET', { case: 'UP' }),
        url, encode: options.encode,
        headers: getCacheVariantHeaders(requestHeaders),
    });
    const urlHash = inBrowser() ? null : sha256(cacheKey);
    ...
    const fetchOptions = {
        ...defFetchOpt, ...options.fetch || {},
        headers: normalizeHeaders(requestHeaders, headers),
    };
    if (options.timeout) {
