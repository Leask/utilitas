...
const promptChatGPT = async (content, options = {}) => {
    ...
    let [resp, resultText, resultAudio, chunk, resultTools, toolsResult] = [
        await client.chat.completions.create({
            modalities, audio: options?.audio || (
                ...
                tools: options?.functions ?? tools.map(x => x.def),
            } : {}, ...options?.jsonMode ? {
    ...
    if (resp?.choices?.[0]?.message?.tool_calls?.length) {
        toolsResult.push({ role: 'assistant', ...resp.choices[0].message });
        for (const fn of resp.choices[0].message.tool_calls) {
            let func, args, stop = false;
            try {
                args = JSON.parse(fn.function.arguments)
            } catch (err) {
                toolsResult.push({
                    role: 'tool',
                    tool_call_id: fn.id,
                    error: `Function call failed, invalid arguments: ${fn.function.arguments}`
                });
                stop = true;
            }
            if (!stop) {
                func = tools.find(x => insensitiveCompare(
                    x.def.function.name, fn.function.name
                ))?.func;
                if (!func) {
                    toolsResult.push({
                        role: 'tool',
                        tool_call_id: fn.id,
                        error: `Function call failed, invalid function name: ${fn.function.name}`
                    });
                    stop = true;
                }
            }
                    ...
                    toolsResult.push(
                        {
                            role: 'tool',
                            tool_call_id: fn.id,
                            content: JSON.stringify(await func(...Object.values(args)))
                        }
                    );
                } catch (err) {
                    toolsResult.push({
                        role: 'tool',
                        tool_call_id: fn.id,
                        error: `Function call failed: ${err.message}`
                    });
                }
            }
        }
        return await promptChatGPT(content, { ...options || {}, toolsResult });
    }
