...
const [
    ...
    CLAUDE_35_HAIKU, AUDIO, WAV, CHATGPT_MINI, ATTACHMENTS, CHAT, OPENAI_VOICE,
        ...
        'CHATGPT_MINI', '[ATTACHMENTS]', 'CHAT', 'OPENAI_VOICE',
    ];
...
const promptGemini = async (content, options) => {
    ...
    return await handleGeminiResponse(resp, options);
};
...
const talk = async (input, options) => {
    ...
    const audioMode = !!(options?.attachments || []).find(x => (
        _MODEL.supportedAudioTypes || []).includes(x.mime_type)
    ) || undefined;
    log(`Prompt: ${JSON.stringify(input)}`);
    switch (engine) {
        case CHATGPT:
            resp = await promptChatGPT(input, {
                messages, model, audioMode, ...options, attachments,
            });
            break;
        case GEMINI:
...
const prompt = async (input, options) => {
    ...
    // Select ChatGPT if no engine is specified:
    // !egn && options?.fast && clients[OPENAI] && (egn = OPENAI)
