...
import {
    ...
    MIME_TGPP, MIME_PCM16, MIME_OGG, convert, decodeBase64DataURL, getImageSize,
    getMime, getTempPath,
} from './storage.mjs';
...
const [
    OPENAI, GOOGLE, OLLAMA, NOVA, DEEPSEEK_32, MD_CODE, CLOUD_OPUS_46, AUDIO,
    WAV, OPENAI_VOICE, GPT_REASONING_EFFORT, THINK, THINK_STR, THINK_END,
    TOOLS_STR, TOOLS_END, TOOLS, TEXT, OK, FUNC, GPT_54, GPT_IMAGE_15,
    GEMMA_3_27B, ANTHROPIC, ais, MAX_TOOL_RECURSION, LOG, name, user, system,
    assistant, JSON_OBJECT, PROMPT_IS_REQUIRED, k, trimTailing, trimBeginning,
    GEMINI_30_PRO_IMAGE, IMAGE, JINA, SILICONFLOW, SF_DEEPSEEK_32,
    OPENROUTER_API, OPENROUTER, AUTO, TOOL, ONLINE, GEMINI_31_PRO,
    GEMINI_30_FLASH, VEO_31, IMAGEN_4_UPSCALE, ERROR_GENERATING,
    GEMINI_25_FLASH_TTS, GEMINI_25_PRO_TTS, wav, PNG_EXT, GPT_4O_MIMI_TTS,
    GPT_4O_TRANSCRIBE, INVALID_AUDIO, OGG_EXT, ELLIPSIS, TOP_LIMIT, ATTACHMENT,
    ...
    MD_CODE_ESCAPED, VERTEX, LYRIA_002,
        ...
        'claude-opus-4.6', 'audio', 'wav', 'OPENAI_VOICE', 'medium', 'think',
        '<think>', '</think>', '<tools>', '</tools>', 'tools', 'text', 'OK',
        'function', 'gpt-5.4', 'gpt-image-1.5', 'gemma3:27b', 'Anthropic', [],
        30, { log: true }, 'Alan', 'user', { role: 'system' },
        ...
        'gemini-2.5-pro-preview-tts', 'wav', 'png', 'gpt-4o-mini-tts',
        'gpt-4o-transcribe', 'Invalid audio data.', 'ogg', '...', 3,
...
const MODEL_ICONS = {
    [OPENROUTER]: '🔀', [OPENAI]: '⚛️', [GOOGLE]: '♊️', // [JINA]: '✴️',
...
// for (const n of [GPT_54, GEMINI_31_PRO, GEMINI_30_FLASH]) {
...
const assertApiKey = (provider, options) => {
    switch (provider) {
        case VERTEX:
            assert(options?.credentials, `${provider} credentials is required.`);
            assert(options?.project, `${provider} project id is required.`);
            return;
    }
...
const unifyProvider = provider => {
    ...
    for (let type of [OPENROUTER, GOOGLE, OPENAI, OLLAMA, SILICONFLOW, VERTEX]) { // JINA,
...
const init = async (options = {}) => {
    ...
    switch (provider) {
        ...
        case VERTEX:
            assertApiKey(provider, options);
            var { GoogleGenAI } = await need('@google/genai');
            const { GoogleAuth } = await need('google-auth-library');
            ...
            for (let model of models) {
                setupAi({
                    provider, model, client, prompt: promptGoogle, priority,
                    options: { authClient, project: options.project, location },
                });
            }
            break;
        case OPENAI:
        ...
        default:
            ...
            for (let model of models) {
                setupAi({
                    provider: provider || OPENROUTER, model, client,
...
const promptGoogle = async (aiId, prompt, options = {}) => {
    ...
    [IMAGEN_4_UPSCALE, LYRIA_002].includes(M.name) || assertPrompt(prompt);
    ...
    if (M.name === IMAGEN_4_UPSCALE) {
        ...
        const [w, h] = getImageSize(imgBuffer);
        const reqFmt = (options?.upscaleFactor || 'x4').toString().toLowerCase();
        const reqOrigin = parseInt(reqFmt.replace(/x/g, '') || 4);
        const limitMax = Math.floor(Math.sqrt(17000000 / (w * h)));
        assert(limitMax >= 2, `Image (${w}x${h}) is too large for upscaling.`);
        const reqFactor = Math.max(2, Math.min(reqOrigin, limitMax, 4));
        var resp = await client.models.upscaleImage({
            model: M.name, image: {
                imageBytes: await convert(imgBuffer, {
                    ...options || {}, input: BUFFER, expected: BASE64,
                }),
            }, upscaleFactor: `x${reqFactor}`,
            ...
            resp = {
                ...
                images: await Promise.all((
                    ...
                    data: await convert(x.image.imageBytes, {
                        ...options || {}, input: BASE64, suffix: 'png',
        ...
        if (M.name === LYRIA_002) {
            const token = await opts.authClient.getAccessToken();
            const region = 'us-central1'; // opts.location;
            const url = `https://${region}-aiplatform.googleapis.com/v1/projects/${opts.project}/locations/${region}/publishers/google/models/${M.name}:predict`;
            const payload = { instances: [{ prompt }] };
            ...
            if (!response.ok) {
                throwError(`API Error: ${response.status} - ${await response.text()}`);
            }
            const data = await response.json();
            var rawAudio = data?.predictions?.[0]?.bytesBase64Encoded;
            assert(rawAudio, ERROR_GENERATING);
            resp = {
                text: '', audio: {
                    data: await convert(rawAudio, {
...
const initChat = async (options = {}) => {
    ...
    for (const ai of ais.filter(x => ![
        IMAGEN_4_UPSCALE, VEO_31, GPT_4O_TRANSCRIBE, LYRIA_002,
