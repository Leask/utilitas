...
const [
    ...
    THINK, THINK_STR, THINK_END, TOOLS_STR, TOOLS_END, TOOLS, TEXT, THINKING,
    OK, FUNC, GPT_5, GPT_5_MINI, GEMMA_3_27B, ANTHROPIC, v8k, ais,
    MAX_TOOL_RECURSION, LOG, name, user, system, assistant, MODEL, JSON_OBJECT,
    TOOL, GEMINI_EMBEDDING_M, tokenSafeRatio, CONTENT_IS_REQUIRED,
    OPENAI_HI_RES_SIZE, k, kT, m, minute, hour, gb, trimTailing, EBD,
    GEMINI_25_FLASH_IMAGE, IMAGE, JINA, JINA_DEEPSEARCH, JINA_CLIP,
    ...
    OPENROUTER, AUTO,
        ...
        'tools', 'text', 'thinking', 'OK', 'function', 'openai/gpt-5',
        'openai/gpt-5-mini', 'gemma3:27b', 'Anthropic', 7680 * 4320, [], 30,
        { log: true }, 'Alan', 'user', 'system', 'assistant', 'model',
        'json_object', 'tool', 'gemini-embedding-001', 1.1,
...
const buildPrompts = async (model, input, options = {}) => {
    ...
    let [history, content, prompt, _system, _model, _assistant, _history] = [
        null, input, null, { role: system }, { role: MODEL },
        { role: assistant }, null,
    ];
    ...
    const systemPrompt = buildGptMessage(options.systemPrompt, _system);
    const msgBuilder = () => {
        [history, _history] = [[], []];
