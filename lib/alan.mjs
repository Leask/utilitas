...
const packToolMsg = msg => ({ role: 'tool', ...msg });
...
const toolsAnthropic = tools.map(x => ({
    ...x, def: {
        ...x.def, function: {
...
const handleToolsCall = async (msg, options) => {
    ...
    if (msg?.tool_calls?.length) {
        res.push({ role: 'assistant', ...msg });
        for (const fn of msg.tool_calls) {
            try { args = JSON.parse(fn.function.arguments) } catch (err) {
                res.push(packToolMsg({
                    tool_call_id: fn.id,
