...
const [name, user, system, assistant, MODEL, JSON_OBJECT, GPT_REASONING_EFFORT]
    = ['Alan', 'user', 'system', 'assistant', 'model', 'json_object', MEDIUM];
...
const init = async (options) => {
    ...
    switch (provider) {
        ...
        case CLAUDE:
            ...
            break;
        case OLLAMA:
            clients[provider] || (options?.host && (clients[provider] = {
...
const packResp = async (resp, options) => {
    ...
    if (!options?.jsonMode) {
        ...
        if (lines[0] === '<think>' && indexOfEnd !== -1) {
            lines[0] = '```think';
            lines[indexOfEnd] = '```';
            lines.slice(0, indexOfEnd - 1).join('').trim()
