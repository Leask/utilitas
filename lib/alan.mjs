...
const buildGptMessage = (content, options) => {
    ...
    assert(message.content.length, 'Content is required.');
    return message;
};
...
const packResp = async (resp, options) => {
    let { text: txt, audio, references }
        = String.isString(resp) ? { txt: resp } : resp;
    ...
    if (!options?.jsonMode && references?.length && !options?.processing) {
        for (const i in references) {
            const [x, sub] = [references[i], references[i].web.map(
                x => ` [${x.title}](${x.uri})`
            ).join('')];
            txt = `${txt.slice(0, x.endIndex)}${sub}${txt.slice(x.endIndex)}`;
            for (let j = parseInt(i) + 1; j < references.length; j++) {
                references[j].startIndex >= x.endIndex
                    && (references[j].startIndex += sub.length);
                references[j].endIndex >= x.endIndex
                    && (references[j].endIndex += sub.length);
            }
        }
    }
    return [{
...
const packGeminiReferences = (chunks, supports) => {
    ...
    if (chunks?.length && supports?.length) {
        ...
        for (const s of supports) {
            const web = [];
            for (const i in s.groundingChunkIndices) {
                chunks?.[s.groundingChunkIndices[i]]?.web && web.push({
                    ...chunks[s.groundingChunkIndices[i]].web,
                    confidenceScore: s.confidenceScores[i],
                });
            }
            web.length && references.push({ ...s.segment, web });
        }
    }
    return references;
};
