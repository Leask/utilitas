...
const _NEED = ['@google/genai', 'js-tiktoken', 'OpenAI'];

const [
    OPENAI, GEMINI, OLLAMA, GEMINI_25_FLASH, NOVA, DEEPSEEK_R1, MD_CODE,
    TEXT_EMBEDDING_3_SMALL, TEXT_EMBEDDING_3_LARGE, CLOUD_SONNET_45, AUDIO, WAV,
    ATTACHMENTS, OPENAI_VOICE, GPT_REASONING_EFFORT, THINK, THINK_STR,
    THINK_END, TOOLS_STR, TOOLS_END, TOOLS, TEXT, OK, FUNC, GPT_5, GPT_5_MINI,
    GEMMA_3_27B, ANTHROPIC, v8k, ais, MAX_TOOL_RECURSION, LOG, name, user,
    system, assistant, MODEL, JSON_OBJECT, GEMINI_EMBEDDING_M, tokenSafeRatio,
    CONTENT_IS_REQUIRED, OPENAI_HI_RES_SIZE, k, kT, m, minute, hour, gb,
    trimTailing, EBD, GEMINI_25_FLASH_IMAGE, IMAGE, JINA, JINA_DEEPSEARCH,
    JINA_CLIP, GEMINI_25_PRO, SILICONFLOW, SF_DEEPSEEK_R1, MAX_TIRE,
    OPENROUTER_API, OPENROUTER, AUTO,
] = [
        'OpenAI', 'Gemini', 'Ollama', 'google/gemini-2.5-flash-preview-09-2025',
        'nova', 'deepseek-r1', '```', 'text-embedding-3-small',
        'text-embedding-3-large', 'anthropic/claude-sonnet-4.5', 'audio', 'wav',
        '[ATTACHMENTS]', 'OPENAI_VOICE', 'medium', 'think', '<think>',
        '</think>', '<tools>', '</tools>', 'tools', 'text', 'OK', 'function',
        'openai/gpt-5', 'openai/gpt-5-mini', 'gemma3:27b', 'Anthropic',
        7680 * 4320, [], 30, { log: true }, 'Alan', 'user', { role: 'system' },
        'assistant', 'model', 'json_object', 'gemini-embedding-001', 1.1,
        'Content is required.', 2048 * 2048, x => 1024 * x, x => 1000 * x,
        x => 1024 * 1024 * x, x => 60 * x, x => 60 * 60 * x,
        x => 1024 * 1024 * 1024 * x, x => x.replace(/[\.\s]*$/, ''),
        { embedding: true }, 'google/gemini-2.5-flash-image', 'image', 'Jina',
        'jina-deepsearch-v1', 'jina-clip-v2', 'google/gemini-2.5-pro',
        'SiliconFlow', 'Pro/deepseek-ai/DeepSeek-R1', 768 * 768,
        'https://openrouter.ai/api/v1', 'OpenRouter', 'openrouter/auto',
    ];
...
for (const n of [GPT_5, GPT_5_MINI, GEMINI_25_PRO, GEMINI_25_FLASH]) {
...
const buildPrompts = async (model, input, options = {}) => {
    ...
    const systemPrompt = buildGptMessage(options.systemPrompt, system);
    const msgBuilder = () => {
        [history, _history] = [[], []];
...
const handleToolsCall = async (msg, options) => {
    ...
    if (calls.length) {
        ...
        content.length && (content = [{ role: user, parts: content }]);
...
const initChat = async (options = {}) => {
    ...
    const [spTokens, ais] = await Promise.all([countTokens([buildGptMessage(
