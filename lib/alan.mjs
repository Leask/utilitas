...
const [
    ...
    OPENROUTER, AUTO, TOOL,
        ...
        'gpt-5', 'gpt-5-mini', 'gemma3:27b', 'Anthropic', 7680 * 4320, [], 30,
        ...
        768 * 768, 'https://openrouter.ai/api/v1', 'OpenRouter',
        'openrouter/auto', 'tool'
    ];
...
const promptOpenAI = async (aiId, content, options = {}) => {
    ...
    let [
        result, resultAudio, resultImages, event, resultTools, responded,
        modalities,
    ] = [
    ...
    const googleImageMode = model?.source === 'google' && modalities?.has?.(IMAGE);
    [options.audioMimeType, options.suffix] = [MIME_PCM16, 'pcm.wav'];
    const targetModel = `${isOpenrouter(provider, model) ? `${model.source}/` : ''}${options.model}`;
    ...
    for await (event of resp) {
        ...
        const deltaImages = (delta?.images || []).map(x => ({ data: x.image_url.url }));
