...
const talk = async (request, options = {}) => {
    ...
    const packMsg = (opts) => ({
        ...
        spoken: renderText(msgs[options.aiId[0]]?.text || '', {
            noCode: true, noLink: true,
        }).replace(/\[\^\d\^\]/ig, ''),
        audios: msgs.map(x => x.audio && caption(x.audio, '🔊', x.model)).filter(x => x),
        images: msgs.map(x => (x.images || []).map(y => caption(y, '🎨', x.model))).flat(),
        videos: msgs.map(x => (x.videos || []).map(y => caption(y, '🎬', x.model))).flat(),
        annotations: msgs.map(x => x.annotations || []).flat(),
    ...
    return {
        sessionId,
        ...chat,
        ...packMsg({ processing: false }),
    };


    // Object.values(msgs).map(x => x.text).join('').trim()
    //     && await ok({ processing: false });
    // const generated = Object.values(msgs).map(x => x.text).join('\n\n');
    // const tts = audio || packMsg({ tts: true });
