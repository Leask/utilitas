...
const [
    ...
    system, assistant, MODEL, JSON_OBJECT, tokenSafeRatio,
    CONTENT_IS_REQUIRED, OPENAI_HI_RES_SIZE, k, kT, m, minute, hour, gb,
    trimTailing, GEMINI_25_FLASH_IMAGE, IMAGE, JINA, JINA_DEEPSEARCH,
    GEMINI_25_PRO, SILICONFLOW, SF_DEEPSEEK_R1, MAX_TIRE,
    OPENROUTER_API, OPENROUTER, AUTO,
] = [
        'OpenAI', 'Gemini', 'Ollama', 'google/gemini-2.5-flash-preview-09-2025',
        'nova', 'deepseek-r1', '```', 'anthropic/claude-sonnet-4.5',
        'audio', 'wav',
        ...
        'assistant', 'model', 'json_object', 1.1,
        'Content is required.', 2048 * 2048, x => 1024 * x, x => 1000 * x,
        x => 1024 * 1024 * x, x => 60 * x, x => 60 * 60 * x,
        x => 1024 * 1024 * 1024 * x, x => x.replace(/[\.\s]*$/, ''),
        'google/gemini-2.5-flash-image', 'image', 'Jina',
        'jina-deepsearch-v1', 'google/gemini-2.5-pro',
