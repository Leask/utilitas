...
const buildAiId = (provider, model) => [
    provider,
    ...provider === OPENROUTER && model?.source ? [model.source] : [],
    model?.name
].map(x => ensureString(x, { case: 'SNAKE' })).join('_');

const buildAiName = (provider, model) => [
    getProviderIcon(provider), provider,
    `(${provider === OPENROUTER && model.source ? `${model.source}/` : ''}${model.name})`
].join(' ');
...
const promptOpenAI = async (aiId, content, options = {}) => {
    ...
    const source = provider === OPENROUTER ? model?.source : null;
    const targetModel = `${source ? `${source}/` : ''}${options.model}`;
