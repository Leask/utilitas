...
const [
    ...
    GEMINI_31_PRO, GEMINI_38_FLASH, VEO_31, ERROR_GENERATING,
    GEMINI_31_FLASH_TTS, GPT_AUDIO, ELLIPSIS, TOP_LIMIT, ATTACHMENT, PROCESSING,
    CURSOR, LN1, LN2, TOP, DEEP_RESEARCH_MAX, MD_CODE_ESCAPED, LYRIA_3,
    GPT_54_MINI, GEMINI_PRO, GEMINI_FLASH,
] = [
        ...
        'gemini-3.8-flash', 'veo-3.1',
        'Error generating content.', 'gemini-3.1-flash-tts-preview',
...
const MODELS = {
    ...
    [VEO_31]: {
        ...
        supportedMimeTypes: [MIME_PNG, MIME_JPEG], defaultProvider: OPENROUTER,
    },
    // tts/stt models
    [GEMINI_31_FLASH_TTS]: {
        source: GOOGLE, maxInputTokens: k(8), audio: true, fast: true,
        hidden: true, defaultProvider: OPENROUTER,
    },
...
const buildOpenAIClientOptions = (options = {}) => {
    const clientOptions = { ...options };
    for (const key of [
        'provider', 'model', 'modelConfig', 'priority', 'preset', 'debug',
...
const OpenAI = async opts => {
    ...
    return { client: new (lib).OpenAI(buildOpenAIClientOptions(opts)) };
};
...
const streamResp = async (resp, options) => {
    ...
    return (msg?.text || msg?.audio?.data?.length
        || msg?.images?.length || msg?.videos?.length)
...
const packResp = async (resp, options) => {
    ...
    audio = await ignoreErrFunc(async () => ({
        data: isMp3
            ? await convert(audio, {
                expected: BUFFER, ...options, input: BUFFER, suffix: audioSuffix
            })
    ...
    const videos = await Promise.all((resp?.videos || []).map(async x => ({
        ...x, data: await convert(x.data, {
            expected: BUFFER, ...options, input: BUFFER, suffix: 'mp4',
        }),
    })));
...
const promptOpenAI = async (aiId, content, options = {}) => {
    ...
    if (model.video) {
        // OpenRouter expects JSON; the SDK's videos.create uses multipart.
        let resp = await client.post('/videos', { body: {
            aspect_ratio: '16:9', ...options.config,
            model: `${source}/${model.name}`.toLowerCase(),
            prompt: trimText(ensureString(content, { trim: true }), {
                limit: model.maxInputTokens,
            }),
        } });
        if (options.generateRaw) { return resp; }
        const jobId = resp.id;
        assert(jobId, resp.error || ERROR_GENERATING, 500);
        await tryUntil(async () => {
            if (['pending', 'in_progress'].includes(resp.status)) {
                resp = await client.videos.retrieve(jobId);
            }
            assert(!['pending', 'in_progress'].includes(resp.status),
                `Waiting for OpenRouter video generation: ${jobId}`);
        }, { interval: 1000 * 30, maxTry: 20, log });
        assert(resp.status === 'completed' && resp.unsigned_urls?.length,
            resp.error || ERROR_GENERATING, 500);
        if (options.videoRaw) { return resp; }
        resp = await packResp({
            text: '',
            videos: await Promise.all(resp.unsigned_urls.map(async (_, index) => ({
                data: Buffer.from(await (await client.videos.downloadContent(
                    jobId, { index },
                )).arrayBuffer()),
                mime_type: MIME_MP4, jobId,
            }))),
        }, options);
        await streamResp(
    ...
    const speechMode = [GEMINI_31_FLASH_TTS].includes(model.name);
    const history = speechMode ? {} : await buildPrompts(model, content, options);
    ...
    // Speech returns binary audio; chat returns SSE deltas.
    const resp = speechMode ? (await client.audio.speech.create({
        model: isOpenrouter(provider, model)
            ? `${source}/${options.model}`.toLowerCase() : options.model,
        input: trimText(content, { limit: model.maxInputTokens }),
        voice: options.voice || 'Zephyr', response_format: 'pcm',
    })).body : await client.chat.completions.create({
        model: isOpenrouter(provider, model) ? (
            `${model.id || `${source}/${options.model}`}${ext}`.toLowerCase()
    ...
    for await (event of resp) {
        ...
        const audioData = speechMode
            ? event : event?.choices?.[0]?.delta?.audio?.data;
        ...
        const deltaAudio = audioData ? await convert(
            audioData, { input: BASE64, expected: BUFFER }
        ) : Buffer.alloc(0);
    ...
    assert(!speechMode || resultAudio.length, ERROR_GENERATING, 500);
    event = {
