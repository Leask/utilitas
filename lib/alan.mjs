...
const promptOllama = async (content, options = {}) => {
    ...
    for await (chunk of resp) {
        ...
        await ignoreErrFunc(async () => await options?.stream?.(
            await packResp(chunk, { ...options || {}, processing: true })
        ), LOG);
    }
    chunk.message.content = result;
    return await packResp(chunk, options);
};
...
const promptClaude = async (content, options = {}) => {
    ...
    for await (const chunk of resp) {
        ...
        result += deltaText;
        await streamResp({ text: options?.delta ? deltaText : result }, options);
    }
    event = {
...
const promptGemini = async (content, options = {}) => {
    ...
    let [result, references, functionCalls, responded, sys, messages]
    ...
    const client = _client.getGenerativeModel({
        model: options?.model,
        systemInstruction: buildGeminiHistory(INSTRUCTIONS, { role: system }),
