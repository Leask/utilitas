...
const packResp = async (resp, options) => {
    ...
    const audioIsMp3 = [MIME_MP3, MIME_MPEGA, MIME_MPGA].includes(audioMimeType);
    const audioIsPcm = [MIME_PCM, MIME_PCM16].includes(audioMimeType);
    const outputAudioMimeType = audioIsMp3 ? MIME_MPEGA : MIME_WAV;
    audioSuffix = audioIsMp3 ? 'mp3' : 'wav';
    audio = await ignoreErrFunc(async () => ({
        ...
        mime: outputAudioMimeType, mimeType: outputAudioMimeType,
    }));
