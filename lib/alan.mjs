...
import {
    ...
    parseJson, throwError, tryUntil, timeout, mergeAtoB,
} from './utilitas.mjs';
...
const OPENAI_RULES = {
    ...
    supportedDocTypes: [MIME_PDF], supportedAudioTypes: [MIME_WAV],
...
const MODELS = {
    ...
    [IMAGEN_4_ULTRA]: {
        ...
        func: 'generateImages', image: true, defaultProvider: GOOGLE,
    },
    [VEO_31]: {
        ...
        func: 'generateVideos', vision: true, image: true,
    ...
    // tts/stt models
    [GEMINI_25_FLASH_TTS]: {
        source: S_GOOGLE, icon: '🗣️', maxInputTokens: kT(32),
        func: 'generateAudio', audio: true, tts: true, fast: true,
        defaultProvider: GOOGLE,
    },
    [GEMINI_25_PRO_TTS]: {
        source: S_GOOGLE, icon: '🗣️', maxInputTokens: kT(32),
        func: 'generateAudio', audio: true, tts: true,
        defaultProvider: GOOGLE,
    },
    [GPT_4O_MIMI_TTS]: {
        source: S_OPENAI, icon: '🗣️', maxInputTokens: kT(2),
        func: 'generateAudio', audio: true, tts: true, fast: true,
        defaultProvider: OPENAI,
    },
    [GPT_4O_TRANSCRIBE]: {
        source: S_OPENAI, icon: '👂', maxInputTokens: 0,
        func: 'transcribeAudio', stt: true, fast: true, defaultProvider: OPENAI,
    },
...
const promptGoogle = async (aiId, prompt, options = {}) => {
    ...
    switch (M?.func) {
        ...
        case 'generateVideos':
            ...
            break;
        case 'generateAudio': // https://ai.google.dev/gemini-api/docs/speech-generation#voices
...
const promptOpenAI = async (aiId, prompt, options = {}) => {
    ...
    switch (M?.func) {
        case 'generateAudio':
            ...
            const content = await client.audio.speech.create({
            ...
            return await convert(buffer, { suffix: 'ogg', ...options || {} });
            break;
