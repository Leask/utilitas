...
const init = async (options) => {
    ...
    const send = async (message, options, onProgress) => {
        ...
        // We should use `sessions[_session]?.details?.spokenText` but it's too short and infoless for now.
        sessions[_session].spokenText = renderText(
            sessions[_session].response, { noCode: true }
        ).replace(/\[\^\d\^\]/ig, '');
        ...
        sessions[_session].suggestedResponses = sessions[
            _session
        ]?.details?.suggestedResponses?.map?.(s => s.text) || [];
        // console.log(JSON.stringify(sessions[_session]));
