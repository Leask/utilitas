...
const _NEED = ['openai', '@waylaidwanderer/chatgpt-api'];
...
const OPENAI_NOT_READY = 'OpenAI API has not been initialized.';
...
const initOpenAI = async (options) => {
    if (options) {
        const OpenAI = await need('openai');
        openaiClient = new OpenAI(options);
    }
    assert(openaiClient, OPENAI_NOT_READY, 501);
    return openaiClient;
};

const createEmbedding = async (input, options) => {
    assert(openaiClient, OPENAI_NOT_READY, 501);
