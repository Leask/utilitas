...
const init = async options => {
    ...
    const send = async (message, options, onProgress) => {
        const [_session, cur, upd, apd] = [
            options?.session || '_',
            key => get(_session, key),
            (key, val) => set(_session, key, val),
            (key, val) => set(_session, key, cur(key) + val),
        ];
        options?.sessionOverwrite && (
            sessions[_session] = options.sessionOverwrite
        );
        const objSession = { parentMessageId: cur('messageId') };
        let lastContext = cur('context');
        ...
        upd('context', options?.context || lastContext);
